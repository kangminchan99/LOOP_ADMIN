    ...
    <>
      ...
      >
        ...
        <p className="mt-3 text-sm leading-6 text-slate-400">
          삭제 후 복구할 수 없습니다. 댓글·알림 등 연결된 일부 데이터도 함께
          삭제될 수 있습니다. 게시글과 업로드 파일은 별도로 관리해야 합니다.
        </p>
        <label htmlFor={inputId} className="mt-5 block text-sm text-slate-300">
          확인을 위해 유저 ID {userId}을 입력해주세요.
        </label>
        <input
          id={inputId}
          value={confirmation}
        ...
        <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          ...
          <button
            type="button"
            disabled={pending || confirmation.trim() !== String(userId)}
            onClick={remove}
