...
import { deleteAdminUser } from '@/src/features/users/api/delete-admin-user';
import { useRouter, useSearchParams } from 'next/navigation';
import { useId, useRef, useState } from 'react';

type Props = { userId: number; nickname: string };

export function DeleteUserButton({ userId, nickname }: Props) {
  ...
  async function remove() {
    if (inFlight.current || confirmation.trim() !== String(userId)) return;
    ...
    try {
      ...
      // 검색/페이지 크기를 유지. 마지막 페이지가 비면 서버 페이지가 보정.
      const params = new URLSearchParams(searchParams.toString());
      params.set('deleted', '1');
      router.replace(`/users?${params.toString()}`, { scroll: false });
      router.refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : '삭제 결과를 확인하지 못했습니다. 목록을 새로고침해주세요.');
    } finally {
  ...
  return (
    <>
      <button type="button" onClick={openDialog}
        aria-label={`${nickname} (ID ${userId}) 삭제`}
      ...
      <dialog ref={dialog} aria-labelledby={titleId}
        ...
        <h2 id={titleId} className="text-lg font-semibold">유저를 삭제하시겠습니까?</h2>
        <p className="mt-3 break-words text-sm text-slate-200">{nickname} · ID {userId}</p>
        <p className="mt-3 text-sm leading-6 text-slate-400">
          삭제 후 복구할 수 없습니다. 댓글·알림 등 연결된 일부 데이터도 함께 삭제될 수 있습니다.
          게시글과 업로드 파일은 별도로 관리해야 합니다.
        </p>
        <label htmlFor={inputId} className="mt-5 block text-sm text-slate-300">확인을 위해 유저 ID {userId}을 입력해주세요.</label>
        <input id={inputId} value={confirmation} onChange={(event) => setConfirmation(event.target.value)}
          inputMode="numeric" autoComplete="off" disabled={pending}
        ...
        <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          <button type="button" disabled={pending} onClick={() => dialog.current?.close()}
            className="min-h-11 rounded-xl border border-white/20 px-4 py-2 text-sm disabled:opacity-50">취소</button>
          <button type="button" disabled={pending || confirmation.trim() !== String(userId)} onClick={remove}
