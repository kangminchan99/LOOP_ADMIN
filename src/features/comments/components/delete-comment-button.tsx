...
import { deleteAdminComment } from '../api/delete-admin-comment';

type Props = { commentId: number; content: string };

export function DeleteCommentButton({ commentId, content }: Props) {
  ...
  async function remove() {
    // 입력한 댓글 ID가 맞을 때만 한 번 요청.
    if (inFlight.current || confirmation.trim() !== String(commentId)) return;
    ...
    try {
      ...
      router.replace(`/comments?${params.toString()}`, { scroll: false });
      router.refresh();
    } catch (error) {
      setMessage(
  ...
  return (
    <>
      ...
      >
        ...
        <p className="mt-3 whitespace-pre-wrap wrap-break-word text-sm text-slate-200">
          {content} · ID {commentId}
        </p>
        <p className="mt-3 text-sm leading-6 text-slate-400">
          삭제한 댓글은 복구할 수 없습니다. 게시글과 작성자 계정은 삭제되지
          않습니다.
        </p>
        <label htmlFor={inputId} className="mt-5 block text-sm text-slate-300">
          확인을 위해 댓글 ID {commentId}을 입력해주세요.
        </label>
        <input
          id={inputId}
          value={confirmation}
        ...
        <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          ...
          <button
            type="button"
            disabled={pending || confirmation.trim() !== String(commentId)}
            onClick={remove}
